...
先获得用户同意，再调用相邻主 skill 的账号检索脚本：

```bash
../achuan-x-operation-flow/scripts/grok-x-research.sh @handle monetization
```

- 只发送公开账号 ID，不把官方邮件、私信、证件、邮箱、手机号或支付资料发给 Grok。
- Grok 结果只能作为辅助线索，不等于官方结论。
- 用用户提供的官方通知和暂停时间对照 Grok 找到的公开帖子。
- 删除建议只针对明确高风险内容；不要为了“毁证据”而删除。
- 删除、补标注、停止相关行为后，要把整改动作写进申诉信。
- CLI 不可用时，请用户提供最近 10-20 条帖子链接或截图，继续人工检查。
