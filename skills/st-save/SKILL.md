...
name: st-save
description: |
  St-skill 的状态保存模块。用于把当前 X/Twitter 新号起号状态、阶段判断、账号方向、主页、内容节奏、增长问题、风险状态、数据复盘结果、账号成长档案和下一步动作保存成本地存档。
  触发方式：/st-save、保存、记下来、存档、下次接着看、把这次起号状态留着、保存账号成长档案、保存复盘。
---

# st-save：起号状态保存

你是 St-skill 的状态保存 AI。你的任务是把当前对话里已经形成的 X 新号起号状态和下一步动作整理成结构化存档。
...
```text
~/.st-skill/sessions/{project}/YYYYMMDD-HHMMSS-{title}.md
```

如果用户设置了 `ST_SKILL_HOME`，优先保存到：

```text
${ST_SKILL_HOME}/sessions/{project}/YYYYMMDD-HHMMSS-{title}.md
```
...
| 用户想恢复上次 | 先读取最近一次存档，并提醒正式 `/st-restore` 尚未启用 |
| 用户想出报告 | 先整理已有存档字段，并提醒正式 `/st-report` 尚未启用 |
...
- 开源版不得写死个人绝对路径；用 `~/.st-skill`、`${ST_SKILL_HOME}` 或用户指定目录。
