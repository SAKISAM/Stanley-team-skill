...
5. 如果提示资格不足，回到收益资格自查；如果提示收款/KYC，转 `/st-infra`。
...
| 类型 | 常见信号 | 处理 |
|---|---|---|
| 资格缺口 | Premium、地区、verified followers、曝光、Stripe、身份验证不满足 | 转 `/st-infra` 补齐 |
...
| 账号安全 | 异常登录、频繁切 IP、账号被锁/冻结 | 转 `/st-risk-recovery` |
...
- 如果缺支付/认证基础设施，转 `/st-infra`。
- 如果曝光不足，转 `/st-growth-diagnosis`。
